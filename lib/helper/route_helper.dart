...
class RouteHelper {
  ...
  static String getChatRoute({required NotificationBody notificationBody, required User user, required int conversationID, required int index}) {
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: orderDetails, page: () {
      return getRoute(Get.arguments != null ? Get.arguments : OrderDetailsScreen(orderId: int.parse(Get.parameters['id'] ?? '0'), orderModel: ));
    }),
    ...
    GetPage(name: orderSuccess, page: () => getRoute(OrderSuccessfulScreen(
      orderID: Get.parameters['id']!, status: Get.parameters['status']!.contains('success') ? 1 : 0, totalAmount:
    ))),
    ...
    GetPage(name: wallet, page: () => getRoute(WalletScreen(fromWallet: Get.parameters['page'] == 'wallet', key: ))),
